...
vec4 distort(){
	...
	float amp2 = .4 + .3*frq3;
	float nx2 = 10.*amp2*(-.5 + noise(uv/texelSize*vec2(.0002, .2)+31.3414+frq6*100.));
	float ny2 = 10.*amp2*(-.5 + noise(uv/texelSize*vec2(.0002, .2)+11.9414+frq6*100.));
	vec2 duv2 = texelSize * vec2(nx2, ny2) * .32 * noise(uv/texelSize*.01+23.993);

	vec4 tex = texture2D(tex0, uv + duv*1.2 + duv2*zamp); // middle middle -- the actual texel / pixel
...
void main() {
	...
	vec4 res = b + (d-b)*pow(.34+ .2*frq4, 1.5) + n*.18;
